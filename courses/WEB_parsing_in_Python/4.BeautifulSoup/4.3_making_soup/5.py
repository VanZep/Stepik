"""
Цель:
Написать код, который будет обрабатывать HTML-структуру, состоящую из
тегов <p>. Код должен анализировать текст внутри каждого тега и, если
количество символов в тексте (без учета пробелов) является чётным, суммировать
значения атрибутов id и class.

Задача:
Обработать все теги <p> в предоставленной HTML-строке.
Для каждого тега <p> проверить длину текста внутри тега без учета пробелов.
Если длина текста чётная, продолжить обработку.
Например:
text = 'Текст тега с четной длиной без символов пробела'
unspaced_text = 'Тексттегасчетнойдлинойбезсимволовпробела'
Извлечь значения атрибутов id и class из соответствующих тегов. Предполагается,
что значения этих атрибутов представляют собой целые числа.
...
def even_number_chars_filter(tags):
    return list(
        filter(lambda tag: len(tag.text.replace(' ', '')) % 2 == 0, tags)
    )
...
def sum_even_length_ids(html):
    ...
    filtered_tags = even_number_chars_filter(tags)
    total_id_sum = sum(int(tag.get('id', 0)) for tag in filtered_tags)
    total_class_sum = sum(
        sum(int(cls) for cls in tag['class'])
        for tag in filtered_tags if tag.get('class')
    )
    print(
