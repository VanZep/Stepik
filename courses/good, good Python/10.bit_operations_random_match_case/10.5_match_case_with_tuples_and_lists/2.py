"""
Вводятся данные по книге в виде строки через запятую в формате (некоторые
...
book = [
    t[i](x) if t[i] != str else x.strip() for i, x in
...
следующей информации с дополнительными проверками:
- автор, название (автор - не менее 6 символов, название - не менее 10
символов)
...
- автор, название, цена, год издания (цена - положительное число и год издания
от 2020 и выше)

Первый шаблон срабатывает, если есть только автор и название; второй, если
появляется еще и цена; третий, если есть автор, название, год издания, но нет
цены; последний, если имеются все данные.
