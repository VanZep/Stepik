...
султану", "Неизвестный автор", "Вдохновляющая, устрашающая, волнующая картина"))
...
class Museum:
    ...
    def remove_exhibit(self, obj):
        if obj not in self.exhibits:
            return print('Такого экспоната нет в музее')

        self.exhibits.remove(obj)

    def get_info_exhibit(self, indx):
        if not len(self.exhibits) > indx:
            return print('В музее меньше экспонатов, чем вы думаете')

        exhibit = self.exhibits[indx]
        return print(f'Описание экспоната {exhibit.name}: {exhibit.descr}')
...
mus.get_info_exhibit(3)
mus.remove_exhibit('1')
