...
app_3 = AppPhone(
    {"Балакирев": 1234567890, "Сергей": 98450647365, "Работа": 112}
...
class SmartPhone:
    ...
    def add_app(self, app):
        if not self.__is_in_apps(app):
            self.apps.append(app)

    def remove_app(self, app):
        if app not in self.apps:
            return print('Такого приложения нет в смартфоне')
        self.apps.remove(app)

    def __is_in_apps(self, app):
        return isinstance(app, tuple(type(app) for app in self.apps))
